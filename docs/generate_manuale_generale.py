...
table(doc,
      ...
      [
          ...
          ["2", "Cloud Functions deploy (FCM + cron)", "Una volta attivo Blaze, deploy delle Cloud Functions per: (a) invio FCM push ai clienti app quando l'operatore crea promo (GRATIS, parte automatico senza click), (b) cron job giornaliero che marca optInStatus=no i pending oltre 30 giorni. NB: WhatsApp automatici NON sono inclusi qui — serve un servizio separato (WhatsApp Cloud API Meta) a pagamento ~0.07\u20ac/messaggio. Per ora WhatsApp resta manual batch.", "Inclusi nel Blaze", "2-3 h sviluppo"],
          ["3", "WhatsApp Cloud API Meta (opzionale futuro)", "Per inviare WhatsApp totalmente automatici (senza click dell'operatore), serve il servizio Cloud API di Meta. Richiede: (a) verifica Meta Business Manager, (b) approvazione di ogni template promo da parte di Meta (1-2 giorni per template), (c) costo ~0.07\u20ac per messaggio marketing in Italia. Esempi: promo a 2000 opted-in = ~140\u20ac, soft opt-in iniziale a 6700 = ~470\u20ac una tantum. Con questo attivato, anche le risposte SI/STOP arrivano automatiche via webhook (no piu' Inbox manuale).", "~0.07\u20ac/msg", "5-7 gg verifica Meta"],
